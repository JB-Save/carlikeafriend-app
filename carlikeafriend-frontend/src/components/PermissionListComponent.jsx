...
export const PermissionListComponent = () => {
    ...
    return (
        <div id="permission-list-content" className="w-100 py-2"> {/*Contenido principal de la lista de permisos  */}
            <div className="d-flex justify-content-center justify-content-md-end mb-3">
                <Link to="/administration/add-permission" className="text-decoration-none py-2 px-2 rounded-3 btn btn-add text-center">
                    <i className="bi bi-plus-circle-fill me-2"></i> Agregar Permiso
                </Link>
            </div>
            <h2 className="h3 fw-bold text-list text-center mb-4">Lista de Permisos</h2>
            ...
            <div className="card custom-card-shadow rounded-3 p-4">
                {isLoading ? (
                    <div className="text-center my-5">
                        ...
                        <p className="mt-2 admin-panel-text-muted">Cargando los permisos...</p>
                    </div>
                ) : (!allPermissions || allPermissions.length === 0) ? (
                    <div className="text-center admin-panel-text-muted mb-3">No hay permisos disponibles.</div>
                ) : (
