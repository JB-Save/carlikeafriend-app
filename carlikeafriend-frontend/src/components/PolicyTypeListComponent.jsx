...
import { PolicyTypeTableComponent } from './PolicyTypeTableComponent';
...
export const PolicyTypeListComponent = () => {
    ...
    const URL = API_CONFIG.POLICY_TYPES;

    const navigate = useNavigate(); // <-- Usa useNavigate para la navegación

    // Función para cargar los tipos de políticas
    const fetchPolicyTypes = async () => {
        ...
        try {
            ...
            console.error("Error al obtener tipo de políticas: ", error);
    ...
    const deleteFunction = async (policyTypeIdToDelete) => {
        setErrDelete(null);
        setIsDeleting(true);

        try {
            const response = await fetch(`${URL}/${policyTypeIdToDelete}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            if (response.ok) {
                setErrDelete("Tipo de política eliminado exitosamente.");
                fetchPolicyTypes(); // Recargar la lista
            } else {
                const msg = await extractErrorMessage(response);
                throw new Error(msg);
            }
        } catch (error) {
            console.error("Error al eliminar tipo de política: ", error);
            const message = error.message.includes("Failed to fetch") ? "No se pudo establecer conexión con el servidor." : error.message;
            setErrDelete(message || "Ocurrió un error inesperado.");
        } finally {
            setIsDeleting(false);
            setPolicyTypeIdToDelete(null);
        }
    };

    return (
        <div id="policy-type-list-content" className="w-100 py-2"> {/*Contenido principal de la lista */}
            <div className="d-flex justify-content-center justify-content-md-end mb-3">
                <Link to="/administration/add-policyType" className="text-decoration-none py-2 px-2 rounded-3 btn btn-add text-center">
                    <i className="bi bi-plus-circle-fill me-2"></i> Agregar Tipo de Política
                </Link>
            </div>
            <h2 className="h3 fw-bold text-list text-center mb-4">Lista Tipo de Políticas</h2>
            ...
            <div className="card custom-card-shadow rounded-3 p-4">
                {isLoading ? (
                    <div className="text-center my-5">
                        ...
                        <p className="mt-2 admin-panel-text-muted">Cargando el tipo de políticas...</p>
                    </div>
                ) : (!allPolicyTypes || allPolicyTypes.length === 0) ? (
                    <div className="text-center admin-panel-text-muted mb-3">No hay tipo de políticas disponibles.</div>
                ) : (
                    <PolicyTypeTableComponent policyTypes={allPolicyTypes} setPolicyTypeIdToDelete={setPolicyTypeIdToDelete} />
                )
                }

            </div>
            {/* El modal de confirmación se renderiza condicionalmente aquí */}
            {policyTypeIdToDelete && (
                <DeleteConfirmationModalComponent
                    id={policyTypeIdToDelete}
                    deleteFunction={deleteFunction}
                    onClose={() => setPolicyTypeIdToDelete(null)}
                    objectName="este tipo de política"
