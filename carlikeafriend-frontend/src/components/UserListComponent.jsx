...
export const UserListComponent = () => {
    ...
    const deleteFunction = async (userIdToDelete) => {
        setErrDelete(null);
        setIsDeleting(true);

        try {
            const response = await fetch(`${URL}/${userIdToDelete}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            if (response.ok) {
                setErrDelete("Usuario eliminado exitosamente.");
