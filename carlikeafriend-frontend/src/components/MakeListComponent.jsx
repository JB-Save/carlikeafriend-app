...
export const MakeListComponent = () => {
    ...
    const deleteFunction = async (makeIdToDelete) => {
        setErrDelete(null);
        setIsDeleting(true);

        try {
            const response = await fetch(`${URL}/${makeIdToDelete}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            if (response.ok) {
                setErrDelete("Marca del producto eliminada exitosamente.");
                fetchMakes(); // Recargar la lista
            } else {
                const msg = await extractErrorMessage(response);
                throw new Error(msg);
            }
    ...
    return (
        <div id="make-list-content" className="w-100 py-2"> {/*Contenido principal de la lista */}
            <div className="d-flex justify-content-center justify-content-md-end mb-3">
                <Link to="/administration/add-make" className="text-decoration-none py-2 px-2 rounded-3 btn btn-add text-center">
                    <i className="bi bi-plus-circle-fill me-2"></i> Agregar Marca
                </Link>
            </div>
            <h2 className="h3 fw-bold text-list text-center mb-4">Lista de Marcas</h2>
            ...
            <div className="card custom-card-shadow rounded-3 p-4">
                {isLoading ? (
                    <div className="text-center my-5">
                        ...
                        <p className="mt-2 admin-panel-text-muted">Cargando las marcas...</p>
                    </div>
                ) : (!allMakes || allMakes.length === 0) ? (
                    <div className="text-center admin-panel-text-muted mb-3">No hay marcas disponibles.</div>
                ) : (
                    <MakeTableComponent makes={allMakes} setMakeIdToDelete={setMakeIdToDelete} />
                )
                }
            ...
            {makeIdToDelete && (
                <DeleteConfirmationModalComponent
                    ...
                    onClose={() => setMakeIdToDelete(null)}
                    objectName="esta marca"
