...
export const VehicleListComponent = () => {
    ...
    const deleteFunction = async (vehicleIdToDelete) => {
        setErrDelete(null);
        setIsDeleting(true);

        try {
            const response = await fetch(`${URL}/${vehicleIdToDelete}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            if (response.ok) {
                setErrDelete("Vehículo eliminado exitosamente.");
                fetchVehicles();
            } else {
    ...
    return (
        <div id="vehicle-list-content" className="w-100 py-2"> {/*Contenido principal de la lista  */}
            <div className="d-flex justify-content-center justify-content-md-end mb-3">
                <Link to="/administration/add-vehicle" className="text-decoration-none py-2 px-2 rounded-3 btn btn-add text-center">
                    <i className="bi bi-plus-circle-fill me-2"></i> Agregar Vehículo
                </Link>
            </div>
            <h2 className="h3 fw-bold text-list text-center mb-4">Lista de Vehículos</h2>
            ...
            <div className="card custom-card-shadow rounded-3 p-4">
                ...
                {isLoading ? (
                    <div className="text-center my-5">
                        ...
                        <p className="mt-2 admin-panel-text-muted">Cargando vehículos...</p>
                    </div>
                ) : (!allVehicles || allVehicles.length === 0) ? (
                    <div className="text-center admin-panel-text-muted mb-3">No hay vehículos disponibles.</div>
                ) : (
                    <VehicleTableComponent vehicles={allVehicles} setVehicleIdToDelete={setVehicleIdToDelete} />
                )
                }
