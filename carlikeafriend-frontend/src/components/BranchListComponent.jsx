...
export const BranchListComponent = () => {
    ...
    const deleteFunction = async (branchIdToDelete) => {
        setErrDelete(null);
        setIsDeleting(true);

        try {
            const response = await fetch(`${URL}/${branchIdToDelete}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            if (response.ok) {
                setErrDelete("Sucursal eliminada exitosamente.");
                fetchBranches();
            } else {
    ...
    return (
        <div id="branch-list-content" className="w-100 py-2"> {/*Contenido principal de la lista  */}
        <div className="d-flex justify-content-center justify-content-md-end mb-3">
            <Link to="/administration/add-branch" className="text-decoration-none py-2 px-2 rounded-3 btn btn-add text-center">
                <i className="bi bi-plus-circle-fill me-2"></i> Agregar Sucursal
            </Link>
            </div>
            <h2 className="h3 fw-bold text-list text-center mb-4">Lista de Sucursales</h2>
            ...
            <div className="card custom-card-shadow rounded-3 p-4">
                ...
                {isLoading ? (
                    <div className="text-center my-5">
                        ...
                        <p className="mt-2 admin-panel-text-muted">Cargando sucursales...</p>
                    </div>
                ) : (!allBranches || allBranches.length === 0) ? (
                    <div className="text-center admin-panel-text-muted mb-3">No hay sucursales disponibles.</div>
                ) : (
                    <BranchTableComponent branches={allBranches} setBranchIdToDelete={setBranchIdToDelete} />
                )
                }
            ...
            {branchIdToDelete && (
                <DeleteConfirmationModalComponent
                    ...
                    onClose={() => setBranchIdToDelete(null)}
                    objectName="esta sucursal"
