...
public class FeatureController {
    ...
    @GetMapping("/features")
    public ResponseEntity<List<FeatureResponseDTO>> findAllFeatures() {
        return new ResponseEntity<>(featureService.findAllFeatures(), HttpStatus.OK);
    }


    @GetMapping("/features/{id}")
    public ResponseEntity<FeatureResponseDTO> findFeatureById(@PathVariable Long id) {
        Optional<FeatureResponseDTO> featureDTO = featureService.findFeatureById(id);
